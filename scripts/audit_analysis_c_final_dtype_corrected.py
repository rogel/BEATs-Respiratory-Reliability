#!/usr/bin/env python3
"""Versioned Analysis C QA correction: preserve statistics, reconstruct producer dtypes."""
...
AMENDMENT = c.BASE / "analysis_c_audit_dtype_amendment_2026_09_06.json"
SPEC = c.REPORTS / "21_ANALYSIS_C_AUDIT_DTYPE_AMENDMENT_2026-09-06.md"
ORIGINAL_AUDIT_SHA = "334cf22b6889e9e4efe0475247b5778a17e297b11408f4faa7746a9ee8840005"
ORIGINAL_RESULT_SHA = "907d0627c2b8142dcfad0f1b49c3d221e57f74f3c1cef07cf354a6508d5bfce0"


def numerical_comparison(left, right):
    left, right = np.asarray(left), np.asarray(right)
    assert left.shape == right.shape and left.ndim == 1 and len(left) > 0
...
def reconstruct_comparison(left_csv, right_csv, stored, *, right_was_float32, same_path):
    # Reproduce the original producer operands, not a new probability estimate.
    left_csv = np.asarray(left_csv, dtype=np.float64)
    right_csv = np.asarray(right_csv, dtype=np.float64)
    direct = numerical_comparison(left_csv, right_csv)
    producer = numerical_comparison(left_csv.astype(np.float32),
                                    right_csv.astype(np.float32) if right_was_float32 else right_csv)
    for name in ("maximum_absolute_probability_difference", "mean_absolute_probability_difference"):
        np.testing.assert_allclose(stored[name], producer[name], rtol=0, atol=1e-14)
    for name in ("events_above_1e_6", "changed_0_5_decisions"):
        assert stored[name] == producer[name], name
    # Serialization must not change the recorded decisions, including cross-path cases.
    assert direct["changed_0_5_decisions"] == producer["changed_0_5_decisions"]
    if same_path:
        for values in (direct, producer):
            assert values["maximum_absolute_probability_difference"] <= 1e-6
            assert values["changed_0_5_decisions"] == 0
    return {"producer_dtypes": ["float32", "float32" if right_was_float32 else "float64"],
            "same_path_gate_required": same_path, "csv_float64_recomputation": direct,
            "original_producer_dtype_reconstruction": producer}
...
def audit_replay(manifest):
    ...
    for seed in c.ALLOWED_SEEDS:
        for domain in c.b.ROLES:
            old = checked_input(c._run_dir(c.ROOT, c.FAMILY, seed) /
                                f"best_validation_predictions_{domain}.csv", manifest, domain, "validation_select")
            frames = {mode: checked_input(c.VALIDATION / f"seed{seed}_{domain}_{mode}.csv",
                                         manifest, domain, "validation_select") for mode in c.MODES}
            record = audit["records"][f"{seed}/{domain}"]
            pairs = [(mode, frames[mode], old, record["modes"][mode], False,
                      mode in (c.MODES[0], c.MODES[2])) for mode in c.MODES]
            pairs.append(("threads1_vs_training_path", frames[c.MODES[2]], frames[c.MODES[0]],
                          record["threads1_vs_training_path"], True, True))
            for mode, left, right, stored, right_was_float32, same_path in pairs:
                values = reconstruct_comparison(left.probability_1.to_numpy(), right.probability_1.to_numpy(),
                    stored, right_was_float32=right_was_float32, same_path=same_path)
                assert stored["changed_0_5_decisions"] == int(np.sum(left.prediction != right.prediction))
                comparisons.append({"seed": seed, "domain": domain, "comparison": mode, **values})
...
def freeze_amendment():
    ...
    assert not (c.OUTPUT / "independent_result_audit.json").exists()
    c.verify()
    c.verify_replay()
    original = c.ROOT / "scripts/audit_analysis_c_final.py"
    result = c.OUTPUT / "analysis_c_final.json"
    assert sha256_file(original) == ORIGINAL_AUDIT_SHA
    assert sha256_file(result) == ORIGINAL_RESULT_SHA
    paths = [Path(__file__).resolve(), original, SPEC, c.FREEZE, result,
             c.ROOT / "tests/test_analysis_c_audit_dtype.py",
             c.REPORTS / "20_ANALYSIS_C_AUDIT_PRECISION_PAUSE_2026-09-06.md",
             c.BASE / "analysis_c_csv_roundtrip_diagnostic_2026_09_06/diagnostic.json",
             c.ROOT / "scripts/diagnose_analysis_c_csv_roundtrip.py",
             c.VALIDATION / "audit_final.json", c.PRED / "prediction_manifest.json",
             c.JOB / "status.json"]
    paths.extend(c.JOB / f"{stage}.log" for stage in ("validate", "predict", "analyze"))
    c.write_json(AMENDMENT, {
        "status": "frozen_before_corrected_independent_audit_after_outcome_computation",
        "frozen_at": c.now(), "reason": "reconstruct original validation diagnostic producer dtypes",
        "author_rule": "clear evidence-backed steps proceed; ask only for unresolved dilemmas",
        "same_path_probability_gate": 1e-6, "diagnostic_identity_atol": 1e-14,
        "classification_threshold": .5, "original_result_sha256": ORIGINAL_RESULT_SHA,
        "no_retraining_or_model_inference": True, "no_statistical_results_changed": True,
        "files": {str(path): sha256_file(path) for path in paths}})
    print({"freeze": str(AMENDMENT), "sha256": sha256_file(AMENDMENT), "files": len(paths)}, flush=True)


def verify_amendment():
    value = c.read_json(AMENDMENT)
    assert value["status"] == "frozen_before_corrected_independent_audit_after_outcome_computation"
    assert value["same_path_probability_gate"] == 1e-6 and value["diagnostic_identity_atol"] == 1e-14
    assert value["classification_threshold"] == .5
    assert value["original_result_sha256"] == ORIGINAL_RESULT_SHA
...
def main():
    ...
    c.write_json(c.OUTPUT/"independent_result_audit.json",{"status":"independent_analysis_c_QA_passed","audited_at":c.now(),
        ...
        "validation_path_comparisons_recomputed":len(replay_comparisons),
        "validation_path_comparison_records":replay_comparisons,
        "dtype_amendment_sha256":sha256_file(AMENDMENT),
        "superseded_audit_source_sha256":ORIGINAL_AUDIT_SHA,
        "correction_scope":"validation diagnostic dtypes only; original statistics and inference unchanged","all_prediction_metadata_and_hashes_verified":True,
...
if __name__=="__main__":
    ...
    freeze_amendment() if args.stage == "freeze" else main()
