#!/usr/bin/env python3
"""Assess the SPRSound Task 1-1 pilot without touching official test splits."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--plain-run",
        default="sprsound_pilot_plain_binary_seed20260727",
    )
    parser.add_argument(
        "--repconv-run",
        default="sprsound_pilot_repconv_binary_seed20260727",
    )
    parser.add_argument(
        "--tfdcr-run",
        default="sprsound_pilot_tfdcr_binary_seed20260727",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/tables/sprsound_binary_pilot_gate.json"),
    )
...
def main() -> None:
    ...
    run_names = {
        "plain": args.plain_run,
        "repconv": args.repconv_run,
        "tfdcr": args.tfdcr_run,
    }
    summaries = {
    ...
    payload = {
        "task": "SPRSound Task 1-1 Normal versus Adventitious",
        "runs": run_names,
        "split": "development_split=validation",
        "official_test_model_evaluation_accessed": False,
